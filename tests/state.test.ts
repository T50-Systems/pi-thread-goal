...
describe("goal state", () => {
  ...
  it("preserves token budget and pause reason across lifecycle events", () => {
    const state = apply([
      { action: "create", goalId: "g1", objective: "ship it", now: 1, tokenBudget: 100_000 },
      { action: "evaluation", goalId: "g1", now: 2, reason: "Need more work", usage: { total: 50_000 } },
      { action: "pause", goalId: "g1", now: 3, reason: "token-budget", message: "budget reached" },
    ]);
  ...
  it("clears pause metadata and resets run usage on resume", () => {
    const state = apply([
      { action: "create", goalId: "g1", objective: "ship it", now: 1, tokenBudget: 100_000 },
      { action: "evaluation", goalId: "g1", now: 2, reason: "Need more work", usage: { total: 50_000 } },
      { action: "pause", goalId: "g1", now: 3, reason: "error", message: "auth failed" },
  ...
  it("tracks and clears pending continuations", () => {
    const state = apply([
      { action: "create", goalId: "g1", objective: "ship it", now: 1 },
      { action: "continuation", goalId: "g1", now: 2, pending: true, reason: "queued" },
