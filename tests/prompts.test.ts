...
describe("goal prompts", () => {
	it("tells agents not to stop after one batch item", () => {
		expect(renderGoalContext(batchGoal)).toContain(
			"finishing one item is progress only",
		);
		expect(renderGoalStartPrompt(batchGoal)).toContain(
			"do not stop after reporting one finished item",
		);
		expect(
			renderGoalContinuationPrompt(batchGoal, "issue 330 completed"),
		).toContain("choose the next unfinished item");
	});
